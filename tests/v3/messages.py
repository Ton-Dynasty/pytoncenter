...
from pytoncenter.utils import create_address_mapping, encode_base64, format_trace
...
class TestDecoder:
    ...
    async def test_get_next_transaction_by_message(self):
        ...
        for out_msg in tx.out_msgs:
            ...
            assert len(txs) == 1
            txhashes.add(txs[0].hash)
        assert txhashes == {encode_base64("ae4bf0c6a14506c6440e063d779768d464cabc87b5f552ba04ed31e9d79d9f93"), encode_base64("754ae0725ab41d3c627e8ca992e250cbc3db239e82d02d93a7b4aebba55ba358")}
    ...
    async def test_get_prev_transaction_by_message(self):
        ...
        """
        tx, _ = await self.client.get_transactions(GetTransactionByHashRequest(hash="84b7c9467a0a24e7a59a5e224e9ef8803563621f4710fe8536ae7803fe245d61"))
        assert tx is not None
        txs, _ = await self.client.get_transaction_by_message(GetTransactionByMessageRequest(direction="out", msg_hash=tx.in_msg.hash))
        assert len(txs) == 1
        assert txs[0].hash == encode_base64("f5333b49b754376ff96b0d299dc38142b8b5ca1b046f41704a4814b51700c89d")
