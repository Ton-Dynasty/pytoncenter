...
class GetBlockRequest(BaseModel):
    ...
    shard: Optional[str] = Field(None, description="Block shard id. Must be sent with workchain")
    seqno: Optional[int] = Field(None, description="Block block seqno. Must be sent with workchain and shard")
    start_utime: Optional[PyDatetime] = Field(None, description="Query blocks with generation UTC timestamp after given timestamp")
    end_utime: Optional[PyDatetime] = Field(None, description="Query blocks with generation UTC timestamp before given timestamp")
    start_lt: Optional[int] = Field(None, description="Query blocks with lt >= start_lt")
    ...
    def check_times(cls, values: GetBlockRequest):
        ...
        if start_utime and end_utime and start_utime > end_utime:
            raise ValueError("start_utime must be earlier than end_utime")
        return values
    ...
    def checl_lts(cls, values: GetBlockRequest):
        ...
        if start_lt and end_lt and start_lt > end_lt:
            raise ValueError("start_lt must be earlier than end_lt")
        return values
    ...
    def check_shard_and_seqno(cls, values: GetBlockRequest):
        ...
        if shard and not workchain:
            raise ValueError("workchain is required if shard is specified")
        if seqno and not (workchain and shard):
            raise ValueError("workchain and shard are required if seqno is specified")
        return values
...
class GetMessagesRequest(BaseModel):
    hash: Optional[str] = Field(None, description="Message hash. Acceptable in hex, base64 and base64url forms")
    source: Optional[PyAddress] = Field(None, description="The source account address. Can be sent in hex, base64 or base64url form")
    destination: Optional[PyAddress] = Field(None, description="The destination account address. Can be sent in hex, base64 or base64url form")
    body_hash: Optional[str] = Field(None, description="Message body hash. Acceptable in hex, base64 and base64url forms")
    limit: int = Field(default=128, ge=1, le=256, description="Limit number of queried rows. Use with offset to batch read")
    offset: int = Field(default=0, ge=0, description="Skip first N rows. Use with limit to batch read")
